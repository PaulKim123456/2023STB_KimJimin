...
#블로그 API 설정(Client_ID와 Client_Secret은 개인별로 추가)
searchUrl <- "https://openapi.naver.com/v1/search/blog.xml"
Client_ID <- "LJRxrLIA2yeUWCyRVpLW"
Client_Secret <- "4WDVG9ne8m"

#블로그 URL 작성(UTF-8로 암호화, API 요청할 URL 정의, 검색결과는 100로 요청)
query <- URLencode(iconv("카카오", to="UTF-8"))
...
#각 리스트의 단어들을 하나의 벡터로 통합(use.names=F는 블로그 원문을 제외한다는 의미)
