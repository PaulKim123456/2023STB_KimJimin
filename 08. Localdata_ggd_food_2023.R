...
#3.open_date변수의 range 확인
#na값제외
foodshop<-foodshop%>%
  filter(open_date!= '') %>%
  select(name,type,status,open_date,close_date,address)
table(foodshop$open_date)
...
#6.address변수
foodshop$district<-substr(foodshop$address,4,7)#시 정보를 분리하여 변수 생성
table(foodshop$district)#이상치 확인
foodshop$district <- ifelse(foodshop$district%in%c(",106","6번지","도 밀양","도 영암","별시 강","별시 관","별시 금","별시 노","별시 마","별시 용","별시 은","사회","역시 계","역시 남","역시 미","역시 서"),NA,foodshop$district)#이상치제거
table(foodshop$district)#이상치 확인
...
foodshop %>%
  filter(!is.na(open_date)&status=="영업") %>% #결측치제거, 영업데이터 추출
  filter(type%in%c("경양식","분식","일식","중국식","호프/통닭"))%>%
